		...
		$(document).ready(function () {
			...
			async function peticion(seleccionado) {
				let response = await $.ajax({
					url: "php/ajax/datosAcademiasUsuario.php",
					method: "POST",
					data: {},
					success: function (resultado) {
						...
						/*function dividirString(inputString) {
							...
							// Si no se encuentra el '[' en el string, retornar un array vacío
							if (startIndex === -1) {
							  return [];
							}
						  
							// Ignorar los caracteres antes del '['
							inputString = inputString.slice(startIndex);
